...
class CarDataPreprocessor:
    def __init__(self):
        ...
        self.preprocessor = ColumnTransformer(transformers=[
            ('make', self.make_pipe, ['make']),
            ('model', self.model_pipe, ['model']),
            ('year', self.year_pipe, ['year']),
            ('engine', self.engine_pipe, ['engine']),
            ('fuel', self.fuel_pipe, ['fuel']),
            ('body', self.body_pipe, ['body']),
            ('drivetrain', self.drivetrain_pipe, ['drivetrain']),
        ])

        # Full pipeline including optional cleaning step
        self.pipeline = Pipeline([
            ('cleaning', FunctionTransformer(clean_df_wrapper, validate=False)),
